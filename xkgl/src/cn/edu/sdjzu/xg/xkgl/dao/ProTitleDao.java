...
public class ProTitleDao {
    ...
    public ProTitle findProTitle(String des) throws SQLException {
        //声明目标对象的引用
        ProTitle desiredProTitle = null;
        //获得数据库连接对象
        Connection connection = JdbcHelper.getConn();
        //根据连接对象准备语句对象
        PreparedStatement preparedStatement =
                connection.prepareStatement("SELECT * FROM protitle WHERE description = ?");
        //对预编译语句对象的参数赋值
        ...
        if(resultSet.next()){
            ...
            String no = resultSet.getString("no");
            desiredProTitle=new ProTitle(id,des,no);
        }
        //关闭资源
        JdbcHelper.close(preparedStatement,connection);
        return desiredProTitle;
